...
function Sidebar({ active, onChange, theme: themeName, onThemeChange, counts }) {
  const t = useTheme();
  ...
  return (
    <aside style={{ background: t.sidebarBg, borderRight: `1px solid ${t.border}` }} className="w-[220px] shrink-0 flex flex-col">
      <div style={{ borderBottom: `1px solid ${t.border}` }} className="h-14 px-4 flex items-center gap-2">
        <div style={{ background: t.brand, color: t.brandText, boxShadow: themeName === 'honey' ? `0 1px 0 rgba(0,0,0,0.04), 0 0 0 1px ${t.brandBorder}` : 'none' }} className="w-6 h-6 rounded-md flex items-center justify-center">
        ...
        <div style={{ background: t.surface2, border: `1px solid ${t.border}` }} className="flex rounded-md p-0.5" role="group" aria-label={tr('theme_label')}>
          {themes.map(th => (
            <button key={th.id} onClick={() => onThemeChange(th.id)}
              title={th.label}
              style={{ background: themeName === th.id ? t.surface : 'transparent', color: themeName === th.id ? (th.id === 'honey' ? t.brand : t.text) : t.textDim, boxShadow: themeName === th.id ? '0 1px 2px rgba(0,0,0,0.05)' : 'none' }}
              className="flex-1 h-7 rounded flex items-center justify-center hover:opacity-80">{th.icon}</button>
          ))}
        </div>
        <div style={{ background: t.surface2, border: `1px solid ${t.border}` }} className="flex rounded-md p-0.5">
          {[['en', 'EN'], ['ko', '한']].map(([code, label]) => (
            <button key={code} onClick={() => setLang(code)}
              style={{ background: lang === code ? t.surface : 'transparent', color: lang === code ? t.text : t.textDim, boxShadow: lang === code ? '0 1px 2px rgba(0,0,0,0.05)' : 'none' }}
              className="flex-1 h-6 rounded text-[11px] font-medium">{label}</button>
          ))}
        </div>
