// ---------- App detail (Endpoint + Configure + Test + Logs) ----------
function AppDetail({ app, collections, onBack, onUpdate }) {
  const t = useTheme();
  const { t: tr } = useI18n();
  ...
  const collection = collections.find(c => c.id === app.collection);
  return (
    <div className="flex flex-col h-full">
      <div style={{ background: t.surface, borderBottom: `1px solid ${t.border}` }} className="h-14 px-6 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2"><Tag color="green"><span className="w-1.5 h-1.5 rounded-full" style={{ background: t.success }}/>{tr('apps_live')}</Tag><Btn variant="secondary" size="sm">{I.more}</Btn></div>
      </div>
      <div style={{ background: t.surface, borderBottom: `1px solid ${t.border}` }} className="px-6 flex items-center gap-1 shrink-0">
        {[{ id: 'endpoint', label: tr('tab_endpoint') }, { id: 'configure', label: tr('tab_configure') }, { id: 'test', label: tr('tab_test') }, { id: 'logs', label: tr('tab_logs') }].map(x => (
          <button key={x.id} onClick={() => setTab(x.id)} style={{ color: tab === x.id ? t.text : t.textDim }}
            className="relative h-10 px-3 text-[13px] font-medium hover:opacity-80">{x.label}
...
function EndpointTab({ url, app, collection }) {
  const t = useTheme();
  const { t: tr } = useI18n();
            ...
            <div className="col-span-8">
              <div style={{ background: t.chipBg }} className="flex rounded-md p-0.5 max-w-[360px]">
                {[['bearer', 'Bearer token'], ['header', 'API-Key header'], ['none', 'Public']].map(([id, label]) => (
                  <button key={id} onClick={() => setAuth(id)} style={{ background: auth === id ? t.surface : 'transparent', color: auth === id ? t.text : t.textDim, boxShadow: auth === id ? '0 1px 2px rgba(0,0,0,0.05)' : 'none' }}
...
function InfoCard({ label, value, sub, icon }) {
  const t = useTheme();
  return (
    <Card className="p-4">
...
function ConfigureTab({ draft, setDraft, collections, onSave }) {
  const t = useTheme();
  const { t: tr } = useI18n();
  return (
    <Card>
      {[
        [tr('cfg_name'), tr('cfg_name_d'), <Input value={draft.name} onChange={e => setDraft({ ...draft, name: e.target.value })}/>],
        [tr('cfg_desc'), tr('cfg_desc_d'), <Input value={draft.description} onChange={e => setDraft({ ...draft, description: e.target.value })}/>],
        [tr('cfg_documents'), tr('cfg_documents_d'), (
          <Select value={draft.collection || ''} onChange={e => setDraft({ ...draft, collection: e.target.value })}>
            ...
            {collections.map(c => <option key={c.id} value={c.id}>{c.name} ({c.docs.length})</option>)}
          </Select>
        )],
        [tr('cfg_prompt'), tr('cfg_prompt_d'), <Textarea rows={4} value={draft.prompt} onChange={e => setDraft({ ...draft, prompt: e.target.value })}/>],
        [tr('cfg_model'), tr('cfg_model_d'), <Input value={draft.model} onChange={e => setDraft({ ...draft, model: e.target.value })}/>],
        ...
        [tr('cfg_topk'), tr('cfg_topk_d', draft.topK), <input type="range" min={1} max={20} value={draft.topK} onChange={e => setDraft({ ...draft, topK: +e.target.value })} className="w-full"/>],
        [tr('cfg_temp'), tr('cfg_temp_d', draft.temperature), <input type="range" min={0} max={1} step={0.1} value={draft.temperature} onChange={e => setDraft({ ...draft, temperature: +e.target.value })} className="w-full"/>],
      ].map(([label, hint, control], i) => (
  ...
  return (
    <Card className="flex flex-col h-[640px]">
      ...
      <div style={{ borderTop: `1px solid ${t.borderSoft}` }} className="px-3 py-3 flex items-center gap-2">
        <Input value={msg} onChange={e => setMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder={tr('test_placeholder')}/>
...
function LogsTab() {
  ...
  // Sparkline: 24 hourly buckets of latency p50
...
Object.assign(window, { AppDetail, EndpointTab, ConfigureTab, TestTab, LogsTab, InfoCard });
