// ---------- Create app modal (with presets) ----------
function CreateAppModal({ open, onClose, onCreate, collections }) {
  const t = useTheme();
  const { t: tr } = useI18n();
  ...
  const presetData = APP_PRESETS.find(p => p.id === preset);
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-6" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: t.surface, border: `1px solid ${t.border}` }} className="rounded-lg w-full max-w-[560px] shadow-xl overflow-hidden">
        ...
        <div className="px-5 py-5 min-h-[300px]">
          ...
          {step === 1 && (
            <div>
              ...
              <Input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder={tr('cm_q1_ph')}/>
            </div>
          )}
          {step === 2 && (
            <div>
              ...
              {[{ id: '', name: tr('info_all_docs'), docs: { length: 6 } }, ...collections].map(c => (
...
function App() {
  ...
  const [openAppId, setOpenAppId] = React.useState(null);
  const [openCollId, setOpenCollId] = React.useState(null);
  const [createOpen, setCreateOpen] = React.useState(false);
  const [uploading, setUploading] = React.useState([]);

  React.useEffect(() => { localStorage.setItem('hanimo-theme', themeName); }, [themeName]);
  React.useEffect(() => { if (active !== 'apps') setOpenAppId(null); if (active !== 'collections') setOpenCollId(null); }, [active]);
  ...
  const startUpload = () => {
    ...
    const tick = () => {
      ...
      setUploading(u => u.map(x => x.id === id ? { ...x, stage: stages[i] } : x));
  ...
  const openColl = collections.find(c => c.id === openCollId);
  const counts = { apps: apps.length, documents: docs.length, collections: collections.length };

  return (
    <ThemeCtx.Provider value={theme}>
      <div data-screen-label="01 Dashboard" style={{ background: theme.appBg, color: theme.text, fontFamily: '"Inter", system-ui, sans-serif' }} className="h-screen flex">
        ...
        <main className="flex-1 flex flex-col min-w-0" style={{ background: theme.appBg }}>
          ...
          {active === 'apps' && (openApp
            ? <AppDetail app={openApp} collections={collections} onBack={() => setOpenAppId(null)} onUpdate={u => setApps(a => a.map(x => x.id === u.id ? u : x))}/>
        ...
        <CreateAppModal open={createOpen} onClose={() => setCreateOpen(false)} onCreate={a => setApps(p => [a, ...p])} collections={collections}/>
      </div>
    </ThemeCtx.Provider>
  );
}
