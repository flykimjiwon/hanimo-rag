...
async def create_app(req: AppCreate, _: str = Depends(require_api_key)):
    row = await fetchrow(
        """INSERT INTO modolrag_apps (name, description, system_prompt, llm_model, temperature, max_tokens, top_k, search_mode, collection_id, document_ids)
...
async def list_apps(_: str = Depends(require_api_key)):
    rows = await fetch(
        "SELECT id, name, description, llm_model, is_active, created_at FROM modolrag_apps ORDER BY created_at DESC"
    )
    return {
        "apps": [
            {
...
async def get_app(app_id: str, _: str = Depends(require_api_key)):
    row = await fetchrow("SELECT * FROM modolrag_apps WHERE id = $1::uuid", app_id)
    if not row:
...
async def update_app(app_id: str, req: AppUpdate, _: str = Depends(require_api_key)):
    ...
    vals.append(app_id)
    await execute(
        f"UPDATE modolrag_apps SET {', '.join(sets)}, updated_at = now() WHERE id = ${len(vals)}::uuid", *vals
    )
...
async def delete_app(app_id: str, _: str = Depends(require_api_key)):
    await execute("DELETE FROM modolrag_apps WHERE id = $1::uuid", app_id)
