...
SYSTEM_PROMPT = (
    "You are ModolRAG assistant. Answer the user's question based ONLY on the provided context. "
...
async def _retrieve(query: str, mode: str, top_k: int, collection_id: str | None, use_hyde: bool):
    ...
    if use_hyde:
        ...
        from modolrag.core.hyde import hyde_transform
    ...
    if collection_id:
        ...
        rows = await db_fetch(
            "SELECT document_id FROM modolrag_collection_documents WHERE collection_id = $1::uuid", collection_id
        )
