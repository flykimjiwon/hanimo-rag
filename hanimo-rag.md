# hanimo-rag — 오픈소스 RAG 엔진 프로젝트 구축
...
>
> **Deliverables**:
> - hanimo-rag Python 패키지 (pip install hanimo-rag)
...
### Original Request
사용자가 hanimo-webui(기존 AI 챗 플랫폼)에 RAG 기능을 추가하고자 함. 기존 프레임워크(LangChain, LlamaIndex)를 사용하지 않고 직접 오픈소스를 제작하여 hanimo-webui에 연동. Simple RAG(벡터+FTS)와 Graph RAG(Obsidian 스타일 지식 그래프) 둘 다 포함.
...
- DB: 같은 PostgreSQL + `hanimo-rag_` 프리픽스 테이블. Supabase 호환
- 인증: X-API-Key 헤더 방식
...
- DB 분리 전략 → 같은 DB + `hanimo-rag_` 프리픽스로 결정
- API 인증 → X-API-Key 방식으로 결정
- hanimo-webui 기존 rag_* 테이블과의 충돌 → hanimo-rag_ 프리픽스로 공존
...
### Core Objective
`/Users/kimjiwon/Desktop/kimjiwon/hanimo-rag/` 에 완전한 RAG 엔진 프로젝트를 구축. pip 설치 가능하고, Docker Compose로 즉시 실행 가능하며, hanimo-webui와 HTTP API로 연동 가능한 상태까지.

### Concrete Deliverables
- `hanimo-rag/` Python 패키지 — 코어 엔진 + FastAPI 서버
- `dashboard/` React SPA — 대시보드 프론트엔드
- `docker-compose.yml` — PostgreSQL + hanimo-rag 원클릭 실행
...
### Definition of Done
- [ ] `pip install -e .` 성공
- [ ] `hanimo-rag serve --db $POSTGRES_URI` 로 서버 기동
...
- hanimo-webui 코드 직접 수정 (연동은 가이드 문서로 제공)
...
> hanimo-rag는 **MIT 라이선스**로 배포. 모든 의존성은 MIT/BSD/Apache-2.0 이하 허용적 라이선스만 허용.
...
### hanimo-rag의 Graph RAG ≠ LightRAG/GraphRAG
...
│                   Graph RAG 접근 방식 비교                       │
├─────────────────────┬─────────────────┬─────────────────────────┤
│ MS GraphRAG         │ LightRAG        │ hanimo-rag                │
├─────────────────────┼─────────────────┼─────────────────────────┤
...
### hanimo-rag Graph RAG의 핵심 가치: "같은 DB, 같은 트랜잭션"
...
**hanimo-rag 해결**: 벡터 + FTS + 그래프가 **하나의 PostgreSQL에서 하나의 트랜잭션**으로 동작
```sql
BEGIN;
  -- 1. 벡터 검색으로 시드 청크 추출
...
## hanimo-webui Integration Bridge

### 기존 hanimo-webui RAG 인프라 현황

hanimo-webui에는 이미 3개의 RAG 관련 테이블이 존재:

| hanimo-webui 기존 테이블 | 용도 | hanimo-rag 대응 |
|---|---|---|
| `rag_documents` | 문서 저장 (status, vectors JSONB) | `hanimo-rag_documents` |
| `rag_models` | RAG 모델 설정 (선택 문서, 파라미터) | `hanimo-rag_settings` |
| `rag_settings` | 전역 설정 (LanceDB 경로, 청크 크기) | `hanimo-rag_settings` |

### 핵심 불일치와 해결

| 항목 | hanimo-webui 기존 | hanimo-rag | 해결 |
|---|---|---|---|
| 벡터 저장소 | LanceDB (파일 기반) | pgvector (PostgreSQL) | hanimo-rag가 대체. LanceDB 코드 제거 불필요 (공존) |
| 벡터 컬럼 | `vectors` JSONB | `embedding halfvec(1536)` | hanimo-rag는 별도 테이블 (`hanimo-rag_` 프리픽스) |
| 문서 상태 | 동일 (uploaded→processing→indexed) | 동일 | 호환 |
| 임베딩 모델 | nomic-embed-text (768d) | nomic-embed-text (768d) + OpenAI (1536d) | 호환, 확장 |
| API 인증 | JWT + API 토큰 | X-API-Key | hanimo-webui에서 X-API-Key 전달 |

### 연동 아키텍처

```
┌──────────────────────────────────────────────────┐
│ hanimo-webui (Next.js)                                │
│                                                  │
│  채팅 → completions API ─┐                       │
│                          ├→ hanimo-rag 호출 여부 판단│
│  /v1/embeddings ─────────┘                       │
...
│                                                  │
│  DB: 같은 PostgreSQL, hanimo-rag_* 테이블          │
└──────────────────────────────────────────────────┘
```

### 마이그레이션 경로 (기존 hanimo-webui RAG 사용자)

**Phase 1: 공존** (hanimo-rag v1)
- hanimo-webui의 기존 `rag_*` 테이블은 그대로 유지
- hanimo-rag는 `hanimo-rag_*` 프리픽스로 별도 테이블 사용
- 같은 PostgreSQL 인스턴스에서 공존
- hanimo-webui가 hanimo-rag API를 HTTP로 호출

**Phase 2: 마이그레이션 도구** (hanimo-rag v1.x)
- `hanimo-rag migrate --from-hanimo-webui` CLI 명령
- `rag_documents` → `hanimo-rag_documents` 데이터 이전
- LanceDB 벡터 → pgvector 재임베딩 (벡터 포맷 비호환으로 재임베딩 필수)
- 기존 `rag_settings` → `hanimo-rag_settings` 설정 매핑

**Phase 3: 네이티브 통합** (hanimo-webui v-next)
- hanimo-webui 채팅에서 RAG 토글 시 hanimo-rag API 자동 호출
- hanimo-webui 관리자 패널에서 hanimo-rag 설정 프록시
- 문서 T22 (hanimo-webui 연동 가이드)에서 상세 안내

### 기존 API 재활용

hanimo-webui에 이미 존재하는 엔드포인트를 hanimo-rag가 활용:
- **`/v1/embeddings`**: hanimo-rag가 임베딩 생성 시 hanimo-webui의 기존 엔드포인트 호출 가능 (Ollama/OpenAI 라우팅 이미 구현됨)
- **`/v1/rerank`**: 검색 결과 재정렬에 활용 가능
- **API 토큰 시스템**: hanimo-webui의 기존 토큰을 hanimo-rag의 X-API-Key로 브릿지
...
├── T15: CLI 엔트리포인트 (hanimo-rag serve) [quick]
...
└── T22: hanimo-webui 연동 가이드 문서 [writing]
...
- [ ] 1. 프로젝트 스캐폴딩 + pyproject.toml
  ...
  **What to do**:
  - `/Users/kimjiwon/Desktop/kimjiwon/hanimo-rag/` 에 전체 디렉토리 구조 생성
  - `pyproject.toml` 작성 — name: hanimo-rag, Python 3.11+, 의존성: fastapi, uvicorn, asyncpg, pypdf, pdfplumber, python-docx, openpyxl, python-pptx, httpx, pydantic-settings
  - `hanimo-rag/__init__.py`, `hanimo-rag/main.py` (FastAPI 앱 + static 서빙), `hanimo-rag/config.py` (환경변수 기반 설정)
  ...
  **References**:
  - `hanimo-rag/main.py` — FastAPI 앱, `StaticFiles` 마운트로 `static/` 서빙
  - Python 패키지 구조: `pyproject.toml` [build-system], [project], [project.scripts] → `hanimo-rag = "hanimo_rag.main:cli"`

  **Acceptance Criteria**:
  - [ ] `pip install -e .` 성공
  - [ ] `python -c "import hanimo-rag"` 에러 없음
  ...
  **QA Scenarios**:
  ...
  Scenario: pip install 성공
    Tool: Bash
    Steps:
      1. cd /Users/kimjiwon/Desktop/kimjiwon/hanimo-rag
      2. pip install -e ".[dev]"
      3. python -c "import hanimo-rag; print(hanimo-rag.__version__)"
  ...
  Scenario: 디렉토리 구조 검증
    Tool: Bash
    Steps:
      1. find /Users/kimjiwon/Desktop/kimjiwon/hanimo-rag -type f | sort
    Expected Result: hanimo-rag/__init__.py, hanimo-rag/main.py, hanimo-rag/config.py, pyproject.toml, Makefile, LICENSE 존재
    Evidence: .sisyphus/evidence/task-1-structure.txt
  ```

  **Commit**: YES
  - Message: `chore: scaffold hanimo-rag project structure`

- [ ] 2. 프로젝트 문서 (README, ARCHITECTURE, SCHEMA)

  **What to do**:
  - `README.md` — 프로젝트 소개, 핵심 특징, 아키텍처 다이어그램(ASCII), 검색 파이프라인, 프로젝트 구조, 빠른 시작, hanimo-webui 연동 예제, 설계 결정 표
  - `docs/ARCHITECTURE.md` — 상세 아키텍처: 수집 파이프라인 흐름, 검색 파이프라인 흐름, 그래프 구축 과정, RRF 융합 알고리즘, 임베딩 어댑터 패턴, 청킹 전략별 비교, 커뮤니티 감지(k-core)
  - `docs/SCHEMA.md` — 전체 DB 스키마: 테이블 DDL(hanimo-rag_ 프리픽스), 인덱스 전략, 관계 다이어그램, Supabase 호환 노트
  ...
  **QA Scenarios**:
  ...
  Scenario: 문서 존재 확인
    Tool: Bash
    Steps:
      1. wc -l /Users/kimjiwon/Desktop/kimjiwon/hanimo-rag/README.md
      2. wc -l /Users/kimjiwon/Desktop/kimjiwon/hanimo-rag/docs/ARCHITECTURE.md
      3. wc -l /Users/kimjiwon/Desktop/kimjiwon/hanimo-rag/docs/SCHEMA.md
...
- [ ] 3. DB 스키마 + 커넥션 모듈
  ...
  **What to do**:
  - `hanimo-rag/db/schema.sql` — 전체 DDL:
    - `hanimo-rag_documents` (id UUID, file_name, original_name, file_size, mime_type, category, status enum, extracted_text, chunk_count, embedding_model, uploaded_by, timestamps)
    - `hanimo-rag_document_chunks` (id, document_id FK, content text, embedding halfvec(1536), fts tsvector GENERATED, chunk_index, chunk_level, parent_chunk_id, metadata JSONB)
    - `hanimo-rag_graph_nodes` (id UUID, namespace, title, content, embedding halfvec(1536), node_type, properties JSONB, UNIQUE(namespace,title))
    - `hanimo-rag_graph_edges` (id UUID, namespace, source_id FK, target_id FK, relation_type, weight, context_snippet, metadata JSONB, UNIQUE(namespace,source_id,target_id,relation_type))
    - `hanimo-rag_communities` (id UUID, path ltree, summary, node_ids UUID[], level, embedding halfvec(1536), needs_resummary bool)
    - `hanimo-rag_settings` (id, chunk_size default 512, chunk_overlap default 51, embedding_model default 'nomic-embed-text', embedding_dimensions default 768, similarity_top_k default 5, similarity_threshold default 0.7, response_mode, api_keys JSONB)
    - HNSW 인덱스: `ON hanimo-rag_document_chunks USING hnsw (embedding halfvec_cosine_ops)`
    - GIN 인덱스: `ON hanimo-rag_document_chunks USING gin(fts)`
    - 엣지 양방향 인덱스: source_id, target_id 각각
    - ltree GiST 인덱스: communities.path
  - `hanimo-rag/db/connection.py` — asyncpg 커넥션 풀, POSTGRES_URI 환경변수, 자동 스키마 초기화, pgvector/pg_trgm/ltree 익스텐션 활성화
  - `hanimo-rag/db/migrations/` — 빈 디렉토리 (향후 마이그레이션용)
  ...
  **QA Scenarios**:
  ...
  Scenario: 스키마 적용 성공
    ...
    Steps:
      ...
      2. psql $POSTGRES_URI -f hanimo-rag/db/schema.sql
      3. psql $POSTGRES_URI -c "\dt hanimo-rag_*"
  ...
  Scenario: 커넥션 풀 동작
    Tool: Bash
    Steps:
      1. python -c "import asyncio; from hanimo_rag.db.connection import get_pool; asyncio.run(get_pool())"
...
- [ ] 4. 설정 관리 + API Key 인증 미들웨어
  ...
  **What to do**:
  - `hanimo-rag/config.py` 확장 — pydantic-settings 기반: POSTGRES_URI, API_KEYS (콤마 구분), EMBEDDING_PROVIDER (ollama/openai), EMBEDDING_MODEL, EMBEDDING_DIMENSIONS, OLLAMA_BASE_URL, OPENAI_API_KEY, CHUNK_SIZE, CHUNK_OVERLAP, SIMILARITY_TOP_K
  - `hanimo-rag/api/auth.py` — FastAPI Dependency: `X-API-Key` 헤더 검증, 설정의 API_KEYS 목록과 대조, 인증 실패 시 401
  - `hanimo-rag/api/middleware.py` — CORS 미들웨어 (hanimo-webui에서 호출 허용)
  ...
  **QA Scenarios**:
  ...
  Scenario: 유효한 API Key
    Tool: Bash
    Steps:
      1. API_KEYS=test-key hanimo-rag serve &
...
- [ ] 5. 임베딩 어댑터 (Ollama + OpenAI)
  ...
  **What to do**:
  - `hanimo-rag/core/embedder.py` — 어댑터 패턴:
  ...
  **QA Scenarios**:
  ...
  Scenario: Ollama 임베딩 (Ollama 미실행 시 스킵)
    ...
    Steps:
      1. python -c "
         from hanimo_rag.core.embedder import OllamaEmbedder
  ...
  Scenario: 팩토리 함수
    ...
    Steps:
      1. python -c "
         ...
         from hanimo-rag.config import Settings
...
- [ ] 6. 문서 파서 6종
  ...
  **What to do**:
  - `hanimo-rag/parsers/base.py` — `ParserBase(ABC)`: `parse(file_path) -> ParsedDocument(text, metadata, pages[])`
  - `hanimo-rag/parsers/pdf.py` — pypdf(BSD-3) + pdfplumber(MIT): 텍스트 추출 + 테이블 감지. 전부 기본 설치, 선택지 없음
  - `hanimo-rag/parsers/docx.py` — python-docx(MIT): 텍스트+스타일+헤딩 구조
  - `hanimo-rag/parsers/xlsx.py` — openpyxl(MIT): 시트별 텍스트, 셀 데이터 마크다운 테이블로 변환
  - `hanimo-rag/parsers/pptx.py` — python-pptx(MIT): 슬라이드별 텍스트+노트
  - `hanimo-rag/parsers/markdown.py` — 마크다운 파싱, 프론트매터(YAML) 추출, 섹션 분리
  - `hanimo-rag/parsers/text.py` — 플레인텍스트 (인코딩 자동 감지)
  - `hanimo-rag/parsers/__init__.py` — `get_parser(mime_type) -> ParserBase` 팩토리

  **설계 원칙**: 플러그인 시스템 없음, extras 없음. `pip install hanimo-rag` 하면 모든 파서가 즉시 동작. 파서 확장(docling 등)은 v2에서 사용자 요청 시 검토.
...
- [ ] 7. 청킹 엔진
  ...
  **QA Scenarios**:
  ...
  Scenario: Recursive 청킹
    ...
    Steps:
      1. python -c "
         from hanimo_rag.core.chunker import RecursiveChunker
...
- [ ] 8. 벡터 스토어 (pgvector CRUD + HNSW)
  ...
  **What to do**:
  - `hanimo-rag/core/vector_store.py`:
...
- [ ] 10. 그래프 스토어 (노드/엣지 CRUD + CTE 탐색)
  ...
  **What to do**:
  - `hanimo-rag/core/graph_store.py`:
...
- [ ] 11. LLM 엔티티/관계 추출기
  ...
  **What to do**:
  - `hanimo-rag/core/extractor.py`:
    ...
    - LLM 호출: 기존 hanimo-webui 모델서버 또는 직접 Ollama/OpenAI API (httpx)
...
- [ ] 12. 하이브리드 검색 + RRF 융합
  ...
  **What to do**:
  - `hanimo-rag/core/hybrid_search.py`:
...
- [ ] 15. CLI 엔트리포인트
  ...
  **What to do**:
  - `hanimo-rag/cli.py`:
    - `hanimo-rag serve` — uvicorn 서버 기동 (--host, --port, --db, --reload 옵션)
    - `hanimo-rag init-db` — 스키마 초기화
    - `hanimo-rag ingest <file>` — CLI에서 직접 문서 수집
  ...
  **Acceptance Criteria**:
  - [ ] `hanimo-rag serve --help` 동작
  - [ ] `hanimo-rag init-db` 스키마 적용

  **Commit**: YES
  - Message: `feat(cli): add hanimo-rag serve CLI entry point`

- [ ] 16. Docker Compose + Makefile

  **What to do**:
  - `docker-compose.yml`: PostgreSQL 15 (pgvector 확장) + hanimo-rag 서비스, 볼륨 마운트, 헬스체크
...
- [ ] 17. 대시보드 스캐폴딩
  ...
  **What to do**:
  ...
  - `vite.config.ts`: 빌드 outDir → `../hanimo-rag/static/`
...
- [ ] 21. Settings 페이지 + 대시보드 빌드
  ...
  **What to do**:
  ...
  - `npm run build` → `hanimo-rag/static/` 에 빌드 산출물 복사
  - `hanimo-rag/main.py` 에서 static 마운트 확인
  ...
  **QA Scenarios**:
  ...
  Scenario: 대시보드 E2E
    ...
    Steps:
      ...
      2. assert title contains "hanimo-rag"
...
- [ ] 22. hanimo-webui 연동 가이드 문서

  **What to do**:
  - `docs/MODOLAI_INTEGRATION.md`:
    - hanimo-webui completions API에서 hanimo-rag 호출하는 코드 예제
    - 환경변수 설정 가이드 (HANIMO_RAG_URL, HANIMO_RAG_API_KEY)
  ...
  **Commit**: YES
  - Message: `docs: add hanimo-webui integration guide`
...
- T1: `chore: scaffold hanimo-rag project structure`
...
```bash
# 설치
pip install -e .  # Expected: Successfully installed hanimo-rag

# 서버 기동
hanimo-rag serve --db $POSTGRES_URI  # Expected: Uvicorn running on 0.0.0.0:8000
...
# Docker
docker compose up -d  # Expected: hanimo-rag + postgres containers running
...
**배경**: 문서 세트별 검색 범위 한정 기능 요구. 예: "Backend 문서만 검색", "HR 정책만 검색". hanimo-webui 스킬 설정도 적용.
...
1. **컬렉션(Collection) 시스템**
   ...
   스키마 추가:
   - `hanimo-rag_collections` — id, name(UNIQUE), description, created_at
   - `hanimo-rag_collection_documents` — collection_id, document_id (복합 PK, 양방향 CASCADE)
...
| 테이블 | 용도 |
|---|---|
| hanimo-rag_documents | 문서 메타데이터 + 처리 상태 |
| hanimo-rag_document_chunks | 청크 + embedding(halfvec 768) + tsvector(auto) |
| hanimo-rag_graph_nodes | 지식 그래프 엔티티 |
| hanimo-rag_graph_edges | 엔티티 관계 |
| hanimo-rag_communities | 그래프 커뮤니티 |
| hanimo-rag_settings | 전역 설정 (singleton) |
| hanimo-rag_collections | 문서 컬렉션 |
| hanimo-rag_collection_documents | 컬렉션-문서 연결 (junction) |

---

### 2026-03-17 — 실행 환경 + 원커맨드 배포 (v0.5)

**배경**: 로컬 환경에서 `pip install hanimo-rag` 실행 불가 (시스템 Python 3.9, pip 미등록). 포트 충돌 방지 필요. 서비스 전체를 한 번에 띄우는 방법 요구.
...
1. **Dockerfile 멀티스테이지 빌드**
   - Stage 1: Node.js 20 → dashboard `npm run build` → hanimo-rag/static/ 출력
...
2. **docker-compose.yml 강화**
   ...
   - 환경변수 기본값 패턴: `${POSTGRES_PASSWORD:-hanimo-rag}`, `${HANIMO_RAG_PORT:-8000}`
   - 3개 서비스: postgres (pgvector:pg15) + ollama + hanimo-rag
...
6. **인프라**:
   ...
   - Docker: 멀티스테이지 빌드 (Node→Python), pgvector:pg15 + Ollama + hanimo-rag
...
**배경**: 기존 오픈소스 RAG 생태계 대비 hanimo-rag 전략 분석 수행. 8개 경쟁 프로젝트(RAGFlow, LightRAG, R2R, Kotaemon, GraphRAG, Cognee, AnythingLLM, Verba) + pgvector 프로덕션 벤치마크 + hanimo-webui 기존 코드베이스 분석.
...
3. **Graph RAG 차별화 섹션 추가** (`Graph RAG Differentiation`)
   - MS GraphRAG / LightRAG / hanimo-rag 3자 비교 테이블
   - "같은 DB, 같은 트랜잭션" 핵심 메시지
   - PostgreSQL CTE 벤치마크 데이터 (0.09ms @ 10K nodes)

4. **hanimo-webui 연동 브릿지 섹션 추가** (`hanimo-webui Integration Bridge`)
   - 기존 rag_*/chat_files 테이블 ↔ hanimo-rag_* 테이블 매핑
   - 3단계 마이그레이션 (공존 → 마이그레이션 도구 → 네이티브 통합)
   - 기존 /v1/embeddings, /v1/rerank 엔드포인트 재활용 설계

5. **파서 아키텍처 심플화**
   - 초기 과설계(3백엔드 플러그인) → 단일 네이티브 파서로 복원
   - `pip install hanimo-rag` 하면 모든 파서 즉시 동작, 선택지 없음
...
| hanimo-webui 시너지 | ⭐4 | ⭐5 (스키마 브릿지 + 마이그레이션) |
...
**배경**: hanimo-webui에 RAG 기능을 추가하기 위해 별도 오픈소스 프로젝트로 hanimo-rag 기획. 기존 프레임워크(LangChain, LlamaIndex) 미사용, PostgreSQL(pgvector) 기반 자체 구현.
...
- 인증: X-API-Key
- 테이블 프리픽스: `hanimo-rag_`

**산출물**: `hanimo-rag.md` 전략 문서 v0.1 (22개 태스크, 4 웨이브)
