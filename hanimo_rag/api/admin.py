...
async def health():
    ...
    """
    from hanimo_rag import __version__
    return {
        "status": "ok",
        "version": __version__,
        "docs": "/docs",
...
async def get_settings(_: str = Depends(require_api_key)):
    ...
    row = await fetchrow("SELECT * FROM hanimo_rag_settings LIMIT 1")
...
async def update_settings(settings: SettingsUpdate, _: str = Depends(require_api_key)):
    ...
    await execute(
        f"UPDATE hanimo_rag_settings SET {set_clauses}, updated_at = now()",
        *values
    )
