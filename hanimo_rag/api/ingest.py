...
async def ingest_document(
    ...
    await execute(
        """
        INSERT INTO hanimo_rag_documents (id, file_name, original_name, file_size, mime_type, category, status)
    ...
    from hanimo_rag.core.pipeline import ingest_document as run_pipeline
...
async def list_documents(
    ...
    if status:
        rows = await fetch(
            "SELECT id, file_name, original_name, file_size, mime_type, category, status, chunk_count, created_at FROM hanimo_rag_documents WHERE status = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3",
            status, limit, offset
        )
    else:
        rows = await fetch(
            "SELECT id, file_name, original_name, file_size, mime_type, category, status, chunk_count, created_at FROM hanimo_rag_documents ORDER BY created_at DESC LIMIT $1 OFFSET $2",
            limit, offset
        )
...
async def get_document(doc_id: str, _: str = Depends(require_api_key)):
    ...
    row = await fetchrow(
        "SELECT * FROM hanimo_rag_documents WHERE id = $1::uuid", doc_id
    )
    if not row:
...
async def delete_document(doc_id: str, _: str = Depends(require_api_key)):
    ...
    row = await fetchrow("SELECT id FROM hanimo_rag_documents WHERE id = $1::uuid", doc_id)
    if not row:
        raise HTTPException(status_code=404, detail="Document not found")
    await execute("DELETE FROM hanimo_rag_documents WHERE id = $1::uuid", doc_id)
